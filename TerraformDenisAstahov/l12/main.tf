...
resource "aws_eip" "my_static_ip" {
  ...
  // merge позволяет объединить два словаря в один, при этом если есть одинаковые ключи, то значение из второго словаря перезапишет значение из первого словаря
